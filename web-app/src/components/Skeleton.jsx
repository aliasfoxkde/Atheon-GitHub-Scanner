 ...
 */
const baseShimmer = 'animate-pulse bg-gradient-to-r from-gray-700 via-gray-600 to-gray-700 bg-[length:200%_100%] animate-shimmer';

export function Skeleton({ className = '' }) {
  return <div className={`${baseShimmer} rounded ${className}`} />;
}
              ...
              <tr key={r} className="border-t border-gray-700">
                {Array.from({ length: columns }).map((_, c) => (
                  <td key={c} className="px-4 py-3">
                    <div className={`${baseShimmer} h-3 rounded`} style={{ width: `${60 + ((r + c) % 4) * 10}%` }} />
                  </td>
                ))}
              </tr>
            ))}
