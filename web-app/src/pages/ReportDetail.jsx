...
export default function ReportDetail() {
  ...
  const load = useCallback(
    async (signal) => {
      ...
      setLoadError(false);
      try {
        const data = await loadRealScannerData(signal);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-64 text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-2">
          {loadError ? 'Failed to load report' : 'Report not found'}
        </h2>
        <p className="text-gray-400 mb-4">
          {loadError
            ? 'Something went wrong while fetching the report data.'
            : `No report matches "${id}"`}
        </p>
  ...
  const npmUrl = report.package_name
    ? `https://www.npmjs.com/package/${report.package_name}`
    : null;
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-800 rounded-lg border border-gray-700 p-5">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className={`px-2.5 py-1 rounded text-xs font-bold ${getTierColor(tier)}`}>
                Tier {tier}
              </span>
              <span className="px-2.5 py-1 rounded text-xs bg-blue-500/10 text-blue-400 font-medium">
                {lang}
              </span>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className="px-3 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg text-xs font-medium no-print"
            >
              Print
            </button>
            <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-5">
          {[
            ...
            {
              label: 'Stars',
              value: stars > 0 ? stars.toLocaleString() : '—',
              color: 'text-white',
            },
            {
              label: 'Dependencies',
              value: deps > 0 ? deps.toLocaleString() : '—',
              color: 'text-white',
            },
            {
              label: 'Files',
              value: files > 0 ? files.toLocaleString() : '—',
              color: 'text-white',
            },
            {
              label: 'Findings',
              value: totalFindings > 0 ? totalFindings.toLocaleString() : '—',
              color: totalFindings > 0 ? 'text-red-400' : 'text-white',
            },
          ].map((s) => (
      ...
      <div className="bg-gray-800 rounded-lg border border-gray-700">
        ...
        <div className="p-4 sm:p-6">
                ...
                <div className="bg-gray-900 rounded-lg p-5 border border-gray-700">
                  ...
                  <dl className="space-y-3 text-sm">
                    {[
                      ...
                      [
                        'Open Issues',
                        report.open_issues > 0 ? report.open_issues.toLocaleString() : '—',
                      ],
                ...
                <div className="bg-gray-900 rounded-lg p-5 border border-gray-700">
                  ...
                  {report.topics && report.topics.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {report.topics.map((t, i) => (
                        <span
                          key={i}
                          className="px-2.5 py-1 bg-purple-500/10 text-purple-300 border border-purple-500/20 rounded-lg text-xs font-medium"
                        >
                          {t}
                        </span>
                      ))}
                  ...
                  {report.tags && report.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {report.tags.map((t, i) => (
                        <span
                          key={i}
                          className="px-2.5 py-1 bg-gray-700 text-gray-300 rounded-lg text-xs"
                        >
                          {t}
                        </span>
                      ))}
                  ...
                  <div className="space-y-2">
                    ...
                    {npmUrl && (
                      ...
                      >
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M0 7.334v8h6.666v1.332H24V7.334H0zm6.666 6.664H5.334v-4H3.999v4H0v-4c0-1.198 1.065-2.665 2.665-2.665h1.334V4c0-1.198 1.065-2.666 2.667-2.666h2.666c1.6 0 2.665 1.468 2.665 2.666v1.332h1.336C17.935 6.332 19 7.8 19 8.998v4h-2.668v-1.334H19v1.334h-5.334V8.998c0-1.2.535-1.997 1.6-1.997h1.336v3.997H5.334V6.999h1.332c1.067 0 1.6.8 1.6 1.999v4H6.666z" />
                        </svg>
                      ...
                      <div className="flex items-start gap-2 text-xs text-gray-400">
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M8 4H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2v-2M16 4h2a2 2 0 012 2v2m-4 4H8m8 0a2 2 0 01-2 2H8m0 0a2 2 0 01-2-2V8m4 4v8"
                          />
                        </svg>
              ...
              {totalFindings > 0 ? (
                ...
                <div className="bg-gray-900 rounded-lg p-8 text-center text-gray-400 border border-gray-700">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    The quality score reflects overall code characteristics.
                  </p>
                </div>
              )}
            </div>
          ...
          {/* DEPENDENCIES */}
          {activeTab === 'deps' && <DependenciesPanel report={report} lang={lang} />}
            ...
            <div className="space-y-4">
              {metrics && Object.keys(metrics).length > 0 ? (
                ...
                <div className="bg-gray-900 rounded-lg p-8 text-center text-gray-400 border border-gray-700">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Only the overall quality score is recorded.
                  </p>
                </div>
              )}
              ...
              <div className="bg-gray-900 rounded-lg p-5 border border-gray-700">
                ...
                <div className="flex justify-center gap-4 mt-4 text-xs text-gray-400">
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-green-500" /> A: 90+
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-blue-500" /> B: 75–89
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-yellow-500" /> C: 60–74
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full bg-red-500" /> D/F: &lt;60
                  </span>
                </div>
              </div>
            </div>
                ...
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3 text-sm">
                  ...
                  {report.clone_url && (
                    <div className="sm:col-span-2">
                      ...
                      <dd className="text-white text-xs font-mono break-all bg-gray-800 rounded p-2">
                        {report.clone_url}
                      </dd>
                    </div>
                  )}
                  {report.homepage && (
                    <div className="sm:col-span-2">
                      ...
                      <dd className="text-white text-xs break-all bg-gray-800 rounded p-2">
                        {report.homepage}
                      </dd>
                    </div>
                  )}
                  {report.description && (
                    <div className="sm:col-span-2">
                      ...
                      <dd className="text-white text-xs bg-gray-800 rounded p-2">
                        {report.description}
                      </dd>
                    </div>
                  )}
...
function DependenciesPanel({ report, lang }) {
  ...
  const pkgName =
    report.package_name || (report.name && report.name.includes('/') ? null : report.name);
  ...
  useEffect(() => {
    ...
    fetcher(pkgName).then((data) => {
      ...
      if (data) {
        ...
        setError(
          `Could not fetch dependencies for "${pkgName}" from ${isPy ? 'PyPI' : 'npm registry'}.`
        );
      }
      setLoading(false);
    });
      ...
      <div className="bg-gray-900 rounded-lg p-6 text-center text-gray-400 border border-gray-700">
        ...
        {pkgName && (
          <div className="mt-3 flex flex-wrap justify-center gap-2">
            ...
            <a
              href={`https://pypi.org/project/${pkgName}/`}
              target="_blank"
              rel="noopener noreferrer"
              className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white rounded text-xs"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-900 rounded-lg border border-gray-700 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700">
          ...
          <a
            href={`https://www.npmjs.com/package/${pkgName}?active-tab=dependencies`}
            target="_blank"
            rel="noopener noreferrer"
      ...
      {devEntries.length > 0 && (
        <div className="bg-gray-900 rounded-lg border border-gray-700 overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500">
                {showDev ? 'click to hide' : 'click to expand'}
              </span>
              <svg
                className={`w-4 h-4 text-gray-400 transition-transform ${showDev ? 'rotate-180' : ''}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      <p className="text-xs text-gray-500">
        Dependency data sourced live from {lang === 'Python' ? 'PyPI' : 'npm registry'}. Exact
        versions reflect what was published at scan time and may differ from your local install.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2.5 hover:bg-gray-800/50 transition-colors">
      <div className="flex items-center gap-2 min-w-0 flex-1">
        <div
          className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${type === 'dev' ? 'bg-purple-400' : 'bg-blue-400'}`}
        />
        <span className="text-white text-sm font-mono truncate">{name}</span>
      </div>
      <div className="flex items-center gap-3 flex-shrink-0">
        {version && <span className="text-gray-400 text-xs font-mono">{version}</span>}
        <a
          href={npmUrl}
          target="_blank"
          rel="noopener noreferrer"
